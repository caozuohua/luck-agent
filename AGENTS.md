...
- **V2 (`main.py`)** — Goal Runtime. `interface/` (Lark WS + health),
...
- `bash deploy.sh [--update]`: deploy or refresh the V2 VPS environment.
...
Configuration is loaded from `.env` via `settings.py`. Do not commit secrets or local credentials.
Lark and cloud credentials are deployment-specific. V2 LLM is optional: `LLM_BASE_URL`, `LLM_API_KEY`, `LLM_MODEL`; unset means offline FakeLLM.
