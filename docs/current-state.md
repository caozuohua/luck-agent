...
Luck Agent 是基于 Lark 国际版的多云 VPS 运维与手机端个人工作/生活统一入口。
...
当前产品优先服务经常使用手机办公的个人工作和生活场景，目标是让用户随手打开 Lark App 就能完成
记录、查询、整理、学习、提醒和轻量执行。产品按意图而不是按平台接口组织能力。以下六类是当前首批
高频样板，不是封闭的功能边界：

- 工作纪要：从零散消息提取决定、行动项和待跟进事项，并在确认后沉淀；
- 英语学习：翻译、纠错、例句、词汇/错题复习和进展回顾；
- Agent 学习：资料摘要、概念问答、实验记录和知识检索；
- Idea 创意：一句话快速捕获、归类、补全和下一步行动；
- 个人博客：从想法或纪要生成大纲、草稿、预览，再确认发布；
- 日程/提醒：自然语言记录事项和时间，确认后提醒和回顾。

后续可以根据真实使用继续扩展其他个人工作、生活意图，例如临时问答、资料整理、个人资料维护、计划执行、
生活记录等。用户不需要记住命令或底层平台，系统应通过统一入口完成意图识别、路由、必要的信息补充、
确认和结果沉淀。

现有 Lark 消息、卡片、Wiki、Docx、Bitable 和后续日历/任务接入均属于支撑层。当前最大的产品 GAP
不是继续增加 API 数量，而是把高频意图做成低打字量、少步骤、可确认、可回顾的完整闭环。个人内容处理
与 VPS 运维控制面隔离，内容理解不能隐式调用危险工具。

### 当前个人数字资产版图

当前真实资产分为四组：

- 云与主机：GCP、Azure，以及承载当前 Agent 和相关服务的 AWS VPS；
- 基础服务：Hermes、A2A、x-ui、new-api、Mem0 等，分布在上述目标环境中，按服务目录和目标路由管理；
- GitHub 项目：blog、portal 等仓库，后续纳入代码状态、Actions、部署和博客工作流；
- Lark 个人资产：日程和 QPC 个人知识碎片多维表格，分别承担计划安排与碎片知识沉淀。

因此统一入口的资产路由应覆盖“云/服务运维、GitHub 项目、个人知识和日程”四个方向。默认以查询、
...
- 阶段五只读平台能力已接入：`/lark chat` 查询当前消息所在会话的基础元数据，`/lark messages [数量]`
  ...
  授权链接，公网 `/oauth/lark/callback` 通过独立回调监听端口（默认 8090）完成授权码交换，默认只允许
  `wiki:wiki:readonly` 等只读 scope。令牌仅驻留内存，不写日志/SQLite，重启后需要重新授权。已接入只读命令
  ...
  归一化纯文本预览，不返回 document token；`docx:document:readonly` 已加入生产 scope 并完成重新授权，真实 Lark 消息已成功返回
  Docx 纯文本摘要；同时已加入文档块结构摘要，按标题、正文、列表、表格等类型统计块数量，不返回块 ID，结构接口失败时降级为纯文本。
  阶段五第一批 Wiki/Docx/Bitable 只读链路已闭环。已实现 `/lark wiki records <Wiki 链接> [表名]` 记录级摘要：单表自动选择，多表按表名选择，
  最多 5 条，字段值截断并对 token、密码、邮箱、手机号等字段脱敏，不返回 app/table/record ID；尚待真实 Lark 消息验收。
...
Bot 身份用于公共团队资源和消息卡片；涉及个人邮件、日历、私信和个人任务时，应单独设计 User OAuth 与权限范围。平台 API 只围绕真实使用场景按需接入，lark-cli 作为候选平台工具层，暂不进入核心运维控制面。
