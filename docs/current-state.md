...
Luck Agent 是基于 Lark 国际版的多云 VPS 运维与手机端个人工作/学习助手。
...
当前产品优先服务经常使用手机办公的日常和工作场景，按意图而不是按平台接口组织能力：
...
不是继续增加 API 数量，而是把上述意图做成低打字量、少步骤、可确认、可回顾的完整闭环。个人内容处理
...
1. P0 场景工作纪要和 Idea 快速捕获尚未实现为一等意图；需要先定义消息输入、结构化结果、确认保存和回顾体验。
...
7. 旧 SPEC、DOCX 手册和 `docs/superpowers/` 仍保留历史设计，但已不作为当前事实来源；兼容代码进入观察期，满足条件后再删除。
