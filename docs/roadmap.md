...
当前进展：已完成 `/lark chat` 和 `/lark messages [数量]` 两个只读能力；仅读取当前事件会话，消息最多
返回 10 条摘要，不扩大写权限，并已通过 Bot 测试会话的 REST 探测。Wiki 搜索需要 `user_access_token`，
当前暂不引入 OAuth；后续继续按单能力、单权限、单验收推进。
