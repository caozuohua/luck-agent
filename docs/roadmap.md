...
2026-09-24 用户要求完成现有上线版本收尾，停止新增 Lark API 接入。
本决定覆盖下文历史扩展路线：日历、任务、邮件等新 API、新 OAuth scope 和新平台能力均延期，
不再自动推进。现有接口只做必要维护，后续工作以稳定性、结果验证和可复现部署为限。
详情见 [收尾记录](release-closeout-2026-09-24.md)；风险和指标以 [历史评估](luck-agent-history-audit-2026-09-24.md) 为准。

2026-09-25 已制定 [一期可发布开发计划](phase-one-release-plan-2026-09-25.md)：
按生产基线、调用契约、决策验收、通知恢复、记忆边界、候选发布顺序实施。
这是维护范围内的发布加固计划，不重新开放下文的 Lark API 扩展路线。
