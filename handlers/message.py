...
class AgentMessageHandler:
    ...
    async def handle(
        ...
        system     = self.router.build_system_prompt(profile, history)
    ...
    async def _dispatch_tool(
        ...
        if name == "create_blog_post":
            ...
            # 同时发送卡片展示（不等待回复）
            asyncio.create_task(self.reply(
                chat_id,
                card=self.card.workflow_runs(args.get("repo",""), runs)
            ))
            return runs
        ...
        # ── Shell 工具 ──
            ...
            asyncio.create_task(self.reply(
                ...
                card=self.card.shell_output(
                    cmd, result["stdout"], result["returncode"], result["elapsed"]
                )
            ))
            return result
